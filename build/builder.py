...
class Builder:
    ...
    def init(self):
        ...
        self.template_html = f"{self.config['base']}{self.config['root']}template.html"
        ...
        for match in self.input:
            for tasks in self.input[match]:
                ...
                for task in task_matches:
                    ...
                    for path in paths:
                        if task == '-':
                            begin2 = begin.replace("{@href_style}", "href='" + path + "'")
                            begin2 = begin2.replace("{@src_script}", "src='" + path + "'")
                            begin2 = begin2.replace("{@src_image}", "src='" + path + "'")
                            result += begin2
                            result += end
                            # self.save(result, n, task, match)
                        elif task == '+':
                            if not path.endswith('.png'):
                                with (open(self.config["base"] + self.config["root"] + path,
    ...
    def save(self, result, n, task, match):
        ...
        for output in self.outputs['{' + task + '}']:
            out = self.config['base'] + self.config['root'] + output.replace('{@number}', str(n))
